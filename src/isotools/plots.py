...
def plot_bar(df, ax=None, drop_categories=None, legend=True, annotate=True, rot=90, bar_width=.5, colors=None,  **axparams):
    ...
    if colors is None:
        colors = [f'C{i}' for i in range(len(df.index)-len(dcat))]  # plot.bar cannot deal with color=None
...
def plot_str_var_number(str_var_count, group_name:'str', n_multi=10, fig_size=(12, 4), fig_title=None, **axparams):
    '''
    Gerenates a figure with three barplots, depicting the number of genes with a certain number of structural variations,
    ...
    _, axs = plt.subplots(1, 3, figsize=fig_size)
    ...
    for i, feature in enumerate(group_tab.columns.str.split('_').str[1].unique()):
        ...
        n_feature_mask = pd.concat([n_feature_tab[n_feature_tab['n_feature'] < n_multi],
                                    pd.DataFrame({'n_feature': n_multi, 'n_gene': n_feature_tab[n_feature_tab['n_feature'] >= n_multi]['n_gene'].sum()}, index=[0])])
        ...
        x = (n_multi + 2) / 2
        pct_multi = 1 - n_feature_mask['n_gene'].iloc[0] / n_feature_mask['n_gene'].sum()
    ...
    _.suptitle(fig_title, fontsize=12)
    _.tight_layout()
...
def triangle_plot(str_var_tab, ax=None, colors=None, tax_title=None):
    ...
    :param colors: How to color the dots in the triangle plot, either a string, a list, or a dict.
                   ...
                   If a list, there should be only one group of structural variation, and the lengthe of the list should be equal to the number of rows in str_var_tab.
    ...
    if colors is None:
        ...
        assert len(colors) == len(coords), 'the length of colors should be equal to the number of rows in str_var_tab'
        color_scheme = {k: colors for k in groups}
    elif isinstance(colors, dict):
