...
def test_import_bam():
    ...
    for sample in ('CTL', 'VPA'):
        isoseq.add_sample_from_bam(f'tests/data/example_1_{sample}.bam', sample_name=sample, group=sample, platform='SequelII')
...
def test_import_csv_reconstruct():  # reconstruct gene structure from scratch
    ...
    for sample in isoseq.samples:
        assert stab1.loc[sample, 'group'] == stab2.loc[sample, 'group'], 'wrong group after csv import for sample %s' % sample
        assert stab1.loc[sample, 'nonchimeric_reads'] == stab2.loc[sample, 'nonchimeric_reads'], 'wrong number of reads after csv import for sample %s' % sample

    discrepancy = False
    for gene in isoseq.iter_genes(query='EXPRESSED'):
        if (gene.is_annotated and gene.id in remapped_genes) or (gene.id not in isoseq_csv and gene.id not in remapped_genes):
    ...
    for gene_csv in isoseq_csv.iter_genes(query='EXPRESSED'):
        ...
        if len(gene.transcripts) != len(gene_csv.transcripts):
            logger.error('number of transcripts for %s changed after csv import: %s != %s', gene.id, len(gene.transcripts), len(gene_csv.transcripts))
            discrepancy = True
    ...
    with FastaFile('tests/data/example.fa') as genome_fh:
        for gene in isoseq:
            ...
            for transcript in gene.ref_transcripts:
                if transcript['transcript_type'] == 'protein_coding' and 'CDS' in transcript:
                    total[gene.strand] += 1
